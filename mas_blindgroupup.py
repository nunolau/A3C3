#!/usr/bin/python3
"""
Multi-agent Blind Group-Up script.

Creates a GymBGU environment and one thread per agent.  The main thread drives
...
from GymBlindGroupUp import GymBGU
...
# ── Main loop ─────────────────────────────────────────────────────────────────
def run(number_of_agents: int = 11, map_size: int = 5):
    env = GymBGU(number_of_agents=number_of_agents, map_size=map_size)
    ...
    for i, agent in enumerate(agents):
        agent.obs_queue.put(info["state_central"])
    ...
    while True:
        ...
        print(f"[env] reward={reward}  terminal={terminal} observations={observations} info={info}")
